...
#include "vk_sdk_platform.h"	// IWYU pragma: keep
...
const float EYEDIST = 35.0;					// eye distance
...
UpdateScene( )
{
	...
	if( UseRotate )
	{
		if (!Paused)
		{
			...
			Scene.uSceneOrient = glm::rotate(glm::mat4(), (float)glm::radians(0.0), axis);
	...
	else
	{
		...
		Scene.uSceneOrient = glm::scale(Scene.uSceneOrient, glm::vec3(Scale, Scale, Scale));
		Scene.uSceneOrient = glm::rotate(Scene.uSceneOrient, Yrot, glm::vec3(0.0, 1.0, 0.0));
		Scene.uSceneOrient = glm::rotate(Scene.uSceneOrient, Xrot, glm::vec3(1.0, 0.0, 0.0));
		// done this way, the Xrot is applied first, then the Yrot, then the Scale
	}

	// change the object projection:
	Scene.uProjection = glm::perspective( FOV, (double)Width/(double)Height, 0.1, 1000. );
	Scene.uProjection[1][1] *= -1.0;
	Fill05DataBuffer( MySceneUniformBuffer, (void *) &Scene );


	float rot1 = (float)(0.5 * M_PI * Time);	// rotation for arm1, in radians
	float rot2 = 2.0f * rot1; 					// rotation for arm2, in radians
	float rot3 = 2.0f * rot2; 					// rotation for arm3, in radians

	// Arm 1
	glm::vec3 zaxis = glm::vec3(0.0, 0.0, 1.0);
	glm::mat4 m1g = glm::mat4(1.0);			// identity
	m1g = glm::translate(m1g, glm::vec3(0.0, 0.0, 0.0));
	m1g = glm::rotate(m1g, rot1, zaxis);	// [T]*[R]

	// Arm 2
	glm::mat4 m21 = glm::mat4(1.0); 		// identity
	m21 = glm::translate(m21, glm::vec3(2.0 * Arm1.armScale, 0.0, 0.0));
	m21 = glm::rotate(m21, rot2, zaxis); 	// [T]*[R]
	m21 = glm::translate(m21, glm::vec3(0.0, 0.0, 2.0)); // z-offset from previous arm

	// Arm 3
	glm::mat4 m32 = glm::mat4(1.0); 		// identity
	m32 = glm::translate(m32, glm::vec3(2.0 * Arm2.armScale, 0.0, 0.0));
	m32 = glm::rotate(m32, rot3, zaxis); 	// [T]*[R]
	m32 = glm::translate(m32, glm::vec3(0.0, 0.0, 2.0)); // z-offset from previous arm

	Arm1.armMatrix = m1g;			// m1g
	Arm1.armColor  = glm::vec3( 0.0, 1.0, 0.0 );
	Arm1.armScale  = 6.0;

	Arm2.armMatrix = m1g * m21; 	// m2g
	Arm2.armColor  = glm::vec3( 1.0, 0.0, 0.0 );
	Arm2.armScale  = 4.0;

	Arm3.armMatrix = m1g * m21 * m32; // m3g
	Arm3.armColor  = glm::vec3( 0.0, 0.0, 1.0 );
	Arm3.armScale  = 2.0;
