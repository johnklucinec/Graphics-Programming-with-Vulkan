...
main( )
{
	...
	// Transform the normal by arm rotation to fix lighting
	vec3 bNormal = mat3(RobotArm.armMatrix) * aNormal;

	vN = normalize( mat3( Object.uNormal ) * bNormal );		// surface normal vector (now using bNormal)

	vec4 ECposition	= VM * vec4( bVertex, 1.0 );
	vec4 lightPos 	= vec4( Scene.uLightPos.xyz, 1.0 );     // light source in fixed location because not transformed
	vL = normalize( lightPos.xyz - ECposition.xyz );      	// vector from the point to the light

	vec4 eyePos 	= vec4( 0.0, 0.0, 0.0, 1.0 );			// eye position after applying the viewing matrix
