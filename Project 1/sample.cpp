...
// As such, it is deliberately verbose so that it is obvious (as possible, at
// least) what is being done
//
...
//
// There are some pieces of the program that were tossed in to show how to do
// some things they are not necessary in the program #define them to turn them
// on, #undef them to turn them off
//	#undef EXAMPLE_OF_USING_DYNAMIC_STATE_VARIABLES
//
// There are also some spots where options are listed just to show you what
// could have happened here:
//	VkPipelineStageFlags waitAtBottom =
// VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT; 	#ifdef CHOICES
...
// --
// 1. is it because the function wants to consume the contents of that tructure
// or array (IN)? or, 2. is it because that function is going to fill that
// staructure or array (OUT)?
#define IN
#define OUT
#define INOUT
...
const float EYEDIST = 2;            	// eye distance
...
// holds all the information about a data buffer so it can be encapsulated in
// one variable:
...
// VkDebugReportCallbackEXT	ErrorCallback = (VkDebugReportCallbackEXT
// *)VK_NULL_HANDLE;
...
VkCommandBuffer TextureCommandBuffer; // used for transfering texture from
                                      // staging buffer to actual texture buffer
...
struct sporadicBuf
    Sporadic; 				// cpu struct to hold miscellaneous information information
int Mode;     				// 0 = use colors, 1 = use textures, ...
MyBuffer MySceneUniformBuffer;
MyTexture MyGoldTexture;       // gold texture struct
MyTexture MyGoldNormalTexture; // gold texture struct
...
// bool						UseIndexBuffer;			// true = use
// both vertex and index buffer, false = just use vertex buffer
...
// VkBool32			ErrorCallback( VkDebugReportFlagsEXT,
// VkDebugReportObjectTypeEXT, uint64_t, size_t, int32_t, const char *, const
// char *, void * );
...
VkResult Init14GraphicsVertexFragmentPipeline(VkShaderModule, VkShaderModule,
                                              VkPrimitiveTopology,
                                              OUT VkPipeline *);
...
void UpdateScene();
// VkBool32		WarningCallback( VkDebugReportFlagsEXT,
// VkDebugReportObjectTypeEXT, uint64_t, size_t, int32_t, const char *, const
// char *, void * );
...
void InitGraphics() {
  ...
  Init07TextureSampler(&MyGoldTexture);
  Init07TextureBufferAndFillFromBmpFile("brick.bmp", &MyGoldTexture);

  Init07TextureSampler(&MyGoldNormalTexture);
  Init07TextureBufferAndFillFromBmpFile("brick_normal.bmp",
                                        &MyGoldNormalTexture);
...
VkResult Init01Instance() {
...

  {
    ...
    const char *instanceLayersWanted[] = {
        //"VK_LAYER_LUNARG_api_dump",			// turn this on if want
        // to see each function call and its arguments (very slow!)
        "VK_LAYER_LUNARG_core_validation", "VK_LAYER_LUNARG_object_tracker",
        "VK_LAYER_LUNARG_parameter_validation", "VK_LAYER_NV_optimus"};
    ...
    vkEnumerateInstanceLayerProperties(&numLayersAvailable,
                                       (VkLayerProperties *)nullptr);
    InstanceLayers = new VkLayerProperties[numLayersAvailable];
    result =
        vkEnumerateInstanceLayerProperties(&numLayersAvailable, InstanceLayers);
    ...
    for (unsigned int i = 0; i < numLayersAvailable; i++) {
      fprintf(FpDebug, "0x%08x  %2d  '%s'  '%s'\n",
              ...
              InstanceLayers[i].layerName, InstanceLayers[i].description);
    }

    instanceLayersWantedAndAvailable.clear();
    for (uint32_t wanted = 0; wanted < numLayersWanted; wanted++) {
      for (uint32_t available = 0; available < numLayersAvailable;
           available++) {
        if (strcmp(instanceLayersWanted[wanted],
                   InstanceLayers[available].layerName) == 0) {
          instanceLayersWantedAndAvailable.push_back(
              InstanceLayers[available].layerName);
          break;
        }
      }
    }
...

  {
    ...
    uint32_t numExtensionsWanted =
        sizeof(instanceExtensionsWanted) / sizeof(char *);
    ...
    vkEnumerateInstanceExtensionProperties((char *)nullptr,
    ...
    result = vkEnumerateInstanceExtensionProperties(
        (char *)nullptr, &numExtensionsAvailable, InstanceExtensions);
    ...
    for (uint32_t wanted = 0; wanted < numExtensionsWanted; wanted++) {
      for (uint32_t available = 0; available < numExtensionsAvailable;
           available++) {
        if (strcmp(instanceExtensionsWanted[wanted],
                   InstanceExtensions[available].extensionName) == 0) {
          extensionsWantedAndAvailable.push_back(
              InstanceExtensions[available].extensionName);
          break;
        }
      }
    }
...
VkResult Init02CreateDebugCallbacks() {
  ...
  PFN_vkCreateDebugReportCallbackEXT vkCreateDebugReportCallbackEXT =
      (PFN_vkCreateDebugReportCallbackEXT) nullptr;
  *(void **)&vkCreateDebugReportCallbackEXT =
  ...
  result = vkCreateDebugReportCallbackEXT(Instance, IN & vdrcci, PALLOCATOR,
                                          OUT & ErrorCallback);
  REPORT("vkCreateDebugReportCallbackEXT - 1");

  vdrcci.flags = VK_DEBUG_REPORT_WARNING_BIT_EXT |
...
VkBool32 ErrorCallback(VkDebugReportFlagsEXT flags,
                       VkDebugReportObjectTypeEXT objectType, uint64_t object,
                       size_t location, int32_t messageCode,
                       const char *pLayerPrefix, const char *pMessage,
                       void *pUserData) {
  fprintf(FpDebug,
          "ErrorCallback: ObjectType = 0x%0x ; object = %ld ; LayerPrefix = "
          "'%s' ; Message = '%s'\n",
          objectType, object, pLayerPrefix, pMessage);
  return VK_TRUE;
}

VkBool32 WarningCallback(VkDebugReportFlagsEXT flags,
                         VkDebugReportObjectTypeEXT objectType, uint64_t object,
                         size_t location, int32_t messageCode,
                         const char *pLayerPrefix, const char *pMessage,
                         void *pUserData) {
  fprintf(FpDebug,
          "WarningCallback: ObjectType = 0x%0x ; object = %ld ; LayerPrefix = "
          "'%s' ; Message = '%s'\n",
          objectType, object, pLayerPrefix, pMessage);
  return VK_TRUE;
}
...
VkResult Init03PhysicalDeviceAndGetQueueFamilyProperties() {
  ...
  result = vkEnumeratePhysicalDevices(Instance, OUT & PhysicalDeviceCount,
  ...
  for (unsigned int i = 0; i < PhysicalDeviceCount; i++) {
    ...
    if (result != VK_SUCCESS) {
      fprintf(FpDebug,
              "Could not get the physical device properties of device %d\n", i);
      return VK_SHOULD_EXIT;
    }
  ...
  vkGetPhysicalDeviceFormatProperties(PhysicalDevice,
                                      IN VK_FORMAT_R32G32B32A32_SFLOAT, &vfp);
  fprintf(
      FpDebug, "Format VK_FORMAT_R32G32B32A32_SFLOAT: 0x%08x 0x%08x  0x%08x\n",
      vfp.linearTilingFeatures, vfp.optimalTilingFeatures, vfp.bufferFeatures);
  vkGetPhysicalDeviceFormatProperties(PhysicalDevice,
                                      IN VK_FORMAT_R8G8B8A8_UNORM, &vfp);
  fprintf(FpDebug, "Format VK_FORMAT_R8G8B8A8_UNORM: 0x%08x 0x%08x  0x%08x\n",
          vfp.linearTilingFeatures, vfp.optimalTilingFeatures,
          vfp.bufferFeatures);
  vkGetPhysicalDeviceFormatProperties(PhysicalDevice,
                                      IN VK_FORMAT_B8G8R8A8_UNORM, &vfp);
  fprintf(FpDebug, "Format VK_FORMAT_B8G8R8A8_UNORM: 0x%08x 0x%08x  0x%08x\n",
          vfp.linearTilingFeatures, vfp.optimalTilingFeatures,
          vfp.bufferFeatures);
  vkGetPhysicalDeviceFormatProperties(PhysicalDevice,
                                      IN VK_FORMAT_B8G8R8A8_SRGB, &vfp);
  ...
  vkGetPhysicalDeviceQueueFamilyProperties(
      IN PhysicalDevice, &count, OUT(VkQueueFamilyProperties *) nullptr);
  ...
  for (unsigned int i = 0; i < count; i++) {
    fprintf(FpDebug, "\t%d: Queue Family Count = %2d  ;   ", i,
            vqfp[i].queueCount);
...
VkResult Init04LogicalDeviceAndQueue() {
  ...
  float queuePriorities[NUM_QUEUES_WANTED] = {1.};
  ...
  vdqci[0].pQueuePriorities =
      queuePriorities; // array of queue priorities [0.,1.]
  ...
  uint32_t layerCount;
  vkEnumerateDeviceLayerProperties(PhysicalDevice, &layerCount,
                                   (VkLayerProperties *)nullptr);
  VkLayerProperties *deviceLayers = new VkLayerProperties[layerCount];
  result = vkEnumerateDeviceLayerProperties(PhysicalDevice, &layerCount,
                                            deviceLayers);
  ...
  for (unsigned int i = 0; i < layerCount; i++) {
    fprintf(FpDebug, "0x%08x  %2d  '%s'  '%s'\n", deviceLayers[i].specVersion,
            deviceLayers[i].implementationVersion, deviceLayers[i].layerName,
    ...
    vkEnumerateDeviceExtensionProperties(
        PhysicalDevice, deviceLayers[i].layerName, &extensionCount,
        (VkExtensionProperties *)nullptr);
  ...
  vdci.pNext = nullptr;
  vdci.flags = 0;
  vdci.queueCreateInfoCount =
      NUM_QUEUES_WANTED; // # of device queues, each of which can create
                         // multiple queues
  ...
  result = vkCreateLogicalDevice(PhysicalDevice, IN & vdci, PALLOCATOR,
                                 OUT & LogicalDevice);
...
// This just creates the data buffer -- filling it with data uses the
// Fill05DataBuffer function Use this for vertex buffers, index buffers, uniform
// buffers, and textures

VkResult Init05DataBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
                          OUT MyBuffer *pMyBuffer) {
  ...
  vbci.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // can only use CONCURRENT if
  ...
  pMyBuffer->size = size;
  result = vkCreateBuffer(LogicalDevice, IN & vbci, PALLOCATOR,
                          OUT & pMyBuffer->buffer);
  REPORT("vkCreateBuffer");

  VkMemoryRequirements vmr;
  vkGetBufferMemoryRequirements(LogicalDevice, IN pMyBuffer->buffer,
                                OUT & vmr); // fills vmr
  if (Verbose) {
  ...
  result = vkBindBufferMemory(LogicalDevice, pMyBuffer->buffer, IN vdm,
                              0); // 0 is the offset
...
VkResult Init05MyIndexDataBuffer(IN VkDeviceSize size,
                                 OUT MyBuffer *pMyBuffer) {
...
VkResult Init05MyVertexDataBuffer(IN VkDeviceSize size,
                                  OUT MyBuffer *pMyBuffer) {
  VkResult result = Init05DataBuffer(size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                     pMyBuffer); // fills pMyBuffer
...
VkResult Init05UniformBuffer(VkDeviceSize size, MyBuffer *pMyBuffer) {
  VkResult result = Init05DataBuffer(size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                     OUT pMyBuffer); // fills pMyBuffer
  return result;
}
...
VkResult Fill05DataBuffer(IN MyBuffer myBuffer, IN void *data) {
  // the size of the data had better match the size that was used to Init the
  // buffer!

  void *pGpuMemory;
  vkMapMemory(LogicalDevice, IN myBuffer.vdm, 0, VK_WHOLE_SIZE, 0,
              &pGpuMemory); // 0 and 0 are offset and flags
  ...
  //
  // but, we could use vkCmdUpdateBuffer( CommandBuffer, myBuffer.buffer, 0,
  // myBuffer.size, data ); instead, except that this requires use of the
  // CommandBuffer which might not be so bad since we are using the
  // CommandBuffer at that moment to draw anyway
}
...
VkResult Init07TextureSampler(MyTexture *pMyTexture) {
...
#endif
  vsci.unnormalizedCoordinates =
      VK_FALSE; // VK_TRUE means we are use raw texels as the index
                // VK_FALSE means we are uing the usual 0. - 1.

  result = vkCreateSampler(LogicalDevice, IN & vsci, PALLOCATOR,
                           OUT & pMyTexture->texSampler);
...
VkResult Init07TextureBuffer(INOUT MyTexture *pMyTexture) {
  ...
  // *******************************************************************************
  {
    ...
    result = vkCreateImage(LogicalDevice, IN & vici, PALLOCATOR,
                           OUT & stagingImage); // allocated, but not filled
    ...
    vmai.memoryTypeIndex = FindMemoryThatIsHostVisible(
        vmr.memoryTypeBits); // because we want to mmap it
    ...
    result = vkBindImageMemory(LogicalDevice, IN stagingImage, IN vdm,
                               0); // 0 = offset
    ...
    vkGetImageSubresourceLayout(LogicalDevice, stagingImage, IN & vis,
                                OUT & vsl);
  ...
  // *******************************************************************************
  {
    ...
    result = vkCreateImage(LogicalDevice, IN & vici, PALLOCATOR,
                           OUT & textureImage); // allocated, but not filled
    REPORT("vkCreateImage");
    ...
    vmai.memoryTypeIndex = FindMemoryThatIsDeviceLocal(
        vmr.memoryTypeBits); // because we want to sample from it

    VkDeviceMemory vdm;
    result = vkAllocateMemory(LogicalDevice, IN & vmai, PALLOCATOR, OUT & vdm);
    REPORT("vkAllocateMemory");

    result = vkBindImageMemory(LogicalDevice, IN textureImage, IN vdm,
                               0); // 0 = offset
    REPORT("vkBindImageMemory");
  }
  // *******************************************************************************
  ...
  // *******************************************************************************
  {
    ...
    vkCmdPipelineBarrier(TextureCommandBuffer, VK_PIPELINE_STAGE_HOST_BIT,
                         VK_PIPELINE_STAGE_HOST_BIT, 0, 0,
                         (VkMemoryBarrier *)nullptr, 0,
                         (VkBufferMemoryBarrier *)nullptr, 1, IN & vimb);
  }
  // *******************************************************************************
  ...
  // *******************************************************************************
  {
    ...
    vkCmdPipelineBarrier(
        TextureCommandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, (VkMemoryBarrier *)nullptr, 0,
        (VkBufferMemoryBarrier *)nullptr, 1, IN & vimb);
    ...
    vkCmdCopyImage(TextureCommandBuffer, stagingImage,
                   VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, textureImage,
                   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, IN & vic);
  }
  // *******************************************************************************
  ...
  // *******************************************************************************
  {
    ...
    vkCmdPipelineBarrier(TextureCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
  ...
  result = vkCreateImageView(LogicalDevice, IN & vivci, PALLOCATOR,
                             OUT & pMyTexture->texImageView);
...
VkResult Init07TextureBufferAndFillFromBmpFile(IN std::string filename,
                                               OUT MyTexture *pMyTexture) {
  ...
  // int oldNumExtra =  4*(( (3*InfoHeader.biWidth)+3)/4) -
...
VkResult Init08Swapchain() {
  ...
  fprintf(FpDebug, "\tcurrentExtent = %d x %d\n", vsc.currentExtent.width,
          vsc.currentExtent.height);
  fprintf(FpDebug, "\tminImageExtent = %d x %d\n", vsc.minImageExtent.width,
          vsc.minImageExtent.height);
  fprintf(FpDebug, "\tmaxImageExtent = %d x %d\n", vsc.maxImageExtent.width,
          vsc.maxImageExtent.height);
  ...
  result = vkGetPhysicalDeviceSurfaceSupportKHR(
      PhysicalDevice, FindQueueFamilyThatDoesGraphics(), Surface, &supported);
  REPORT("vkGetPhysicalDeviceSurfaceSupportKHR");
  if (supported == VK_TRUE) {
    fprintf(FpDebug, "** This Surface is supported by the Graphics Queue **\n");
  } else {
    fprintf(FpDebug,
            "** This Surface is not supported by the Graphics Queue **\n");
  }

  uint32_t formatCount;
  vkGetPhysicalDeviceSurfaceFormatsKHR(PhysicalDevice, Surface, &formatCount,
                                       (VkSurfaceFormatKHR *)nullptr);
  ...
  < -- -- -the 1080tis can use this one VK_FORMAT_B8G8R8A8_SNORM = 45,
  VK_FORMAT_B8G8R8A8_USCALED = 46, VK_FORMAT_B8G8R8A8_SSCALED = 47,
  VK_FORMAT_B8G8R8A8_UINT = 48, VK_FORMAT_B8G8R8A8_SINT = 49,
  VK_FORMAT_B8G8R8A8_SRGB = 50,
  < -- -- -the 1080 tis can use this one
#endif

#ifdef COLOR_SPACE_CHOICES
              VK_COLOR_SPACE_SRGB_NONLINEAR_KHR = 0,
  < -- -- -the 1080tis use this one VK_COLOR_SPACE_DISPLAY_P3_NONLINEAR_EXT =
      1000104001,
  ...
  for (uint32_t i = 0; i < formatCount; i++) {
    fprintf(FpDebug, "%3d:     %4d     %12d", i, surfaceFormats[i].format,
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(
      PhysicalDevice, Surface, &presentModeCount, (VkPresentModeKHR *)nullptr);
  VkPresentModeKHR *presentModes = new VkPresentModeKHR[presentModeCount];
  vkGetPhysicalDeviceSurfacePresentModesKHR(PhysicalDevice, Surface,
                                            &presentModeCount, presentModes);
#ifdef PRESENT_MODE_CHOICES
  VK_PRESENT_MODE_IMMEDIATE_KHR = 0, VK_PRESENT_MODE_MAILBOX_KHR = 1,
  < -- -- -the 1080tis can handle this VK_PRESENT_MODE_FIFO_KHR = 2,
  < -- -- -the 1080tis can handle this VK_PRESENT_MODE_FIFO_RELAXED_KHR = 3,
  < -- -- -the 1080tis can
              handle this VK_PRESENT_MODE_SHARED_DEMAND_REFRESH_KHR =
      1000111000,
  ...
  vscci.oldSwapchain =
      (VkSwapchainKHR)0; // we're not replacing an old swapchain
  vscci.clipped = VK_TRUE;

  result = vkCreateSwapchainKHR(LogicalDevice, IN & vscci, PALLOCATOR,
                                OUT & SwapChain);
  REPORT("vkCreateSwapchainKHR");

  uint32_t imageCount;
  result = vkGetSwapchainImagesKHR(LogicalDevice, IN SwapChain,
                                   OUT & imageCount, (VkImage *)nullptr); // 0
  REPORT("vkGetSwapchainImagesKHR - 0");
  if (imageCount != 2) {
    fprintf(FpDebug,
            "imageCount return from vkGetSwapchainImages = %d; should have "
            "been 2\n",
            imageCount);
    return result;
  }
  ...
  result = vkGetSwapchainImagesKHR(LogicalDevice, SwapChain, OUT & imageCount,
                                   PresentImages); // 0
...
VkResult Init09DepthStencilImage() {
  ...
  result =
      vkCreateImage(LogicalDevice, IN & vici, PALLOCATOR, &DepthStencilImage);
  REPORT("vkCreateImage");
  ...
  result =
      vkAllocateMemory(LogicalDevice, IN & vmai, PALLOCATOR, OUT & imageMemory);
  REPORT("vkAllocateMemory");

  result = vkBindImageMemory(LogicalDevice, DepthStencilImage, imageMemory,
                             0); // 0 is the offset
  REPORT("vkBindImageMemory");
...
VkResult Init10RenderPasses() {
  ...
  result = vkCreateRenderPass(LogicalDevice, IN & vrpci, PALLOCATOR,
                              OUT & RenderPass);
...
VkResult Init11Framebuffers() {
  ...
  result = vkCreateFramebuffer(LogicalDevice, IN & vfbci, PALLOCATOR,
                               OUT & Framebuffers[0]);
...
VkResult Init06CommandPools() {
...

  {
    ...
    result = vkCreateCommandPool(LogicalDevice, IN & vcpci, PALLOCATOR,
                                 OUT & GraphicsCommandPool);
...
VkResult Init06CommandBuffers() {
...

  {
    ...
    result = vkAllocateCommandBuffers(LogicalDevice, IN & vcbai,
                                      OUT & CommandBuffers[0]);
    REPORT("vkAllocateCommandBuffers - 1");
  }

  // allocate 1 command buffer for the transfering pixels from a staging buffer
  // to a texture buffer:
...
VkResult Init12SpirvShader(std::string filename,
  ...
  if (magic != SPIRV_MAGIC) {
    fprintf(FpDebug,
            "Magic number for spir-v file '%s is 0x%08x -- should be 0x%08x\n",
  ...
  VkResult result =
      vkCreateShaderModule(LogicalDevice, &vsmci, PALLOCATOR, pShaderModule);
...
VkDescriptorType:
  ...
  result = vkCreateDescriptorPool(LogicalDevice, IN & vdpci, PALLOCATOR,
                                  OUT & DescriptorPool);
...
// Each set has a layout, which describes the order and type of data in that
// set. The pipeline layout consists of multiple DS layouts.
...
layout(set = 3, binding = 1) uniform sampler2D uNormalTex;
#endif
...
VkResult Init13DescriptorSetLayouts() {
  ...
  ObjectSet[0].stageFlags =
      VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
  ObjectSet[0].pImmutableSamplers = (VkSampler *)nullptr;

  // DS #3:
  VkDescriptorSetLayoutBinding TexSamplerSet[2];
  // Binding 0: uSampler (gold texture)
  ...
  // Binding 1: uNormalTex (normal texture)
  TexSamplerSet[1].binding = 1;
  TexSamplerSet[1].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  TexSamplerSet[1].descriptorCount = 1;
  TexSamplerSet[1].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
  TexSamplerSet[1].pImmutableSamplers = (VkSampler *)nullptr;
...
VkDescriptorType:
  ...
  vdslc3.flags = 0;
  vdslc3.bindingCount = 2;
  vdslc3.pBindings = &TexSamplerSet[0];

  result = vkCreateDescriptorSetLayout(LogicalDevice, &vdslc0, PALLOCATOR,
                                       OUT & DescriptorSetLayouts[0]);
  REPORT("vkCreateDescriptorSetLayout - 0");

  result = vkCreateDescriptorSetLayout(LogicalDevice, &vdslc1, PALLOCATOR,
                                       OUT & DescriptorSetLayouts[1]);
  REPORT("vkCreateDescriptorSetLayout - 1");

  result = vkCreateDescriptorSetLayout(LogicalDevice, &vdslc2, PALLOCATOR,
                                       OUT & DescriptorSetLayouts[2]);
  REPORT("vkCreateDescriptorSetLayout - 2");

  result = vkCreateDescriptorSetLayout(LogicalDevice, &vdslc3, PALLOCATOR,
                                       OUT & DescriptorSetLayouts[3]);
...
VkResult Init13DescriptorSets() {
  ...
  result = vkAllocateDescriptorSets(LogicalDevice, IN & vdsai,
                                    OUT & DescriptorSets[0]);
  ...
  vdii0.sampler = MyGoldTexture.texSampler;
  vdii0.imageView = MyGoldTexture.texImageView;
  vdii0.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

  VkDescriptorImageInfo vdii1;
  vdii1.sampler = MyGoldNormalTexture.texSampler;
  vdii1.imageView = MyGoldNormalTexture.texImageView;
  ...
  vwds4.pImageInfo = &vdii1; // normal texture
  vwds4.pTexelBufferView = (VkBufferView *)nullptr;

  uint32_t copyCount = 0;

  // this could have been done with one call and an array of
  // VkWriteDescriptorSets:

  vkUpdateDescriptorSets(LogicalDevice, 1, IN & vwds0, IN copyCount,
                         (VkCopyDescriptorSet *)nullptr);
  vkUpdateDescriptorSets(LogicalDevice, 1, IN & vwds1, IN copyCount,
                         (VkCopyDescriptorSet *)nullptr);
  vkUpdateDescriptorSets(LogicalDevice, 1, IN & vwds2, IN copyCount,
                         (VkCopyDescriptorSet *)nullptr);
  vkUpdateDescriptorSets(LogicalDevice, 1, IN & vwds3, IN copyCount,
                         (VkCopyDescriptorSet *)nullptr);
  vkUpdateDescriptorSets(LogicalDevice, 1, IN & vwds4, IN copyCount,
                         (VkCopyDescriptorSet *)nullptr);
...
VkResult Init14GraphicsVertexFragmentPipeline(
  ...
  vprsci.cullMode = VK_CULL_MODE_NONE; // best to do this because of the
  ...
  VkVertexInputBindingDescription
      vvibd[1]; // an array containing one of these per buffer being used
  ...
  VkVertexInputAttributeDescription
      vviad[4]; // an array containing one of these per vertex attribute in all
                // bindings 4 = vertex, normal, color, texture coord
  ...
  VK_FORMAT_VEC4 = VK_FORMAT_R32G32B32A32_SFLOAT VK_FORMAT_XYZW =
      VK_FORMAT_R32G32B32A32_SFLOAT VK_FORMAT_VEC3 =
          VK_FORMAT_R32G32B32_SFLOAT VK_FORMAT_STP =
              VK_FORMAT_R32G32B32_SFLOAT VK_FORMAT_XYZ =
                  VK_FORMAT_R32G32B32_SFLOAT VK_FORMAT_VEC2 =
                      VK_FORMAT_R32G32_SFLOAT VK_FORMAT_ST =
                          VK_FORMAT_R32G32_SFLOAT VK_FORMAT_XY =
                              VK_FORMAT_R32G32_SFLOAT VK_FORMAT_FLOAT =
                                  VK_FORMAT_R32_SFLOAT VK_FORMAT_S =
                                      VK_FORMAT_R32_SFLOAT VK_FORMAT_X =
  ...
  VkPipelineVertexInputStateCreateInfo
      vpvisci; // used to describe the input vertex attributes
...
#endif
  vprsci.cullMode = VK_CULL_MODE_NONE; // recommend this because of the
                                       // projMatrix[1][1] *= -1.;
#ifdef CHOICES
  VK_CULL_MODE_NONE
  ...
  vpcbas.colorWriteMask = VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT |
...
#ifdef CHOICES
  VK_DYNAMIC_STATE_VIEWPORT-- vkCmdSetViewort() VK_DYNAMIC_STATE_SCISSOR-- vkCmdSetScissor()
  ...
  vgpci.pTessellationState =
      (VkPipelineTessellationStateCreateInfo *)nullptr; // &vptsci
  ...
  result =
      vkCreateGraphicsPipelines(LogicalDevice, (VkPipelineCache)0, 1,
                                IN & vgpci, PALLOCATOR, OUT pGraphicsPipeline);
...
VkResult Init14ComputePipeline(VkShaderModule computeShader,
                               OUT VkPipeline *pComputePipeline) {
  ...
  result = vkCreateComputePipelines(LogicalDevice, (VkPipelineCache)0, 1,
                                    &vcpci[0], PALLOCATOR, pComputePipeline);
...
vplci.pNext = nullptr;
vplci.flags = 0;
vplci.setLayoutCount = << length of array.pSetLayouts >> vplci.pSetLayouts =
                       << array of type VkDescriptorSetLayout >>
                          vplci.pushConstantRangeCount =
                       << length of array pPushConstantRanges >>
                          vplci.pPushConstantRanges =
                       << array of type VkPushConstantRange >>

                          result = vkCreatePipelineLayout(LogicalDevice, &vplci,
                                                          PALLOCATOR,
                                                          &PipelineLayout);
#endif

#ifdef SAMPLE_CODE
vkCmdPushConstants(CommandBuffer, PipelineLayout, VK_SHADER_STAGE_ALL, offset,
                   size, void *values);
#endif
...
// The compiler can make code-generation decisions based on Specialization
// Constants. Specialization constants are good for: 	branching (~ #ifdef)
// switch 	loop unrolling 	constant folding 	operator simplification
...
vsme[0].constantId = << the constant_id in the layout line,
    uint32_t >> vsme[0].offset = << how far into the raw data this constant is,
                bytes >> vsme[0].size = << size of this SC in the raw data >>

                                        VkSpecializationInfo vsi;
vsi.mapEntryCount = << number of SCs to be set >>
                    vsi.pmapEntries =
                    << array of VkSpecializationMapEntry elements >>
                       vsi.dataSize = << in bytes >> vsi.pData =
                    << the raw data,
                    void * >>
#endif
...
#ifdef RESULT
VK_SUCCESS:
  its signaled VK_NOT_READY : its not signaled
#endif
                                  REPORT("vkGetFenceStatus");

  result =
      vkWaitForFence(LogicalDevice, fenceCount, pFences, waitForAll, timeout);
#ifdef CHOICES
...
VkResult InitEvent() {
  ...
  VkResult result =
      vkCreateEvent(LogicalDevice, IN & veci, PALLOCATOR, OUT & Event);
...
#ifdef RESULTS
VK_EVENT_SET:
  signaled VK_EVENT_RESET : not signaled
#endif
  ...
  result =
      vkCmdWaitEvents(CommandBuffer, eventCount, pEvents, srcPipelineStageBits,
                      dstPipelineStageBits, memoryBarrierCount, pMemoryBarriers,
  ...
  VkResult InitSemaphore() {
    ...
    result = vkCreateSemaphore(LogicalDevice, IN & vsci, PALLOCATOR,
                               OUT & SemaphoreImageAvailable);
    REPORT("vkCreateSemaphore -- image available");

    result = vkCreateSemaphore(LogicalDevice, IN & vsci, PALLOCATOR,
                               OUT & SemaphoreRenderFinished);
  ...
  VkResult DestroyAllVulkan() {
    ...
    // do not need to free invdividual descriptors because we did not enable
    // VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT in the flags
    ...
    vkFreeCommandBuffers(LogicalDevice, GraphicsCommandPool,
                         sizeof(CommandBuffers) / sizeof(CommandBuffers[0]),
                         CommandBuffers);
    vkFreeCommandBuffers(LogicalDevice, TransferCommandPool, 1,
    ...
    vkDestroyImageView(LogicalDevice, MyGoldTexture.texImageView, PALLOCATOR);
    vkDestroyImage(LogicalDevice, MyGoldTexture.texImage, PALLOCATOR);
    vkDestroySampler(LogicalDevice, MyGoldTexture.texSampler, PALLOCATOR);
    vkFreeMemory(LogicalDevice, MyGoldTexture.stagingMemory, PALLOCATOR);
    vkFreeMemory(LogicalDevice, MyGoldTexture.textureMemory, PALLOCATOR);

    vkDestroyImageView(LogicalDevice, MyGoldNormalTexture.texImageView,
                       PALLOCATOR);
    vkDestroyImage(LogicalDevice, MyGoldNormalTexture.texImage, PALLOCATOR);
    vkDestroySampler(LogicalDevice, MyGoldNormalTexture.texSampler, PALLOCATOR);
    vkFreeMemory(LogicalDevice, MyGoldNormalTexture.stagingMemory, PALLOCATOR);
    vkFreeMemory(LogicalDevice, MyGoldNormalTexture.textureMemory, PALLOCATOR);
    ...
    for (auto descriptorSetLayout : DescriptorSetLayouts)
      vkDestroyDescriptorSetLayout(LogicalDevice, descriptorSetLayout,
                                   PALLOCATOR);
    ...
    vkFreeMemory(LogicalDevice, MyObjectUniformBuffer.vdm, PALLOCATOR);
    vkFreeMemory(LogicalDevice, MyVertexDataBuffer.vdm, PALLOCATOR);
    vkFreeMemory(LogicalDevice, MyGoldTexture.vdm, PALLOCATOR);
    vkFreeMemory(LogicalDevice, MyGoldNormalTexture.vdm, PALLOCATOR);
    ...
    vkDestroyDescriptorPool(LogicalDevice, DescriptorPool, PALLOCATOR);
    vkDestroyDescriptorSetLayout(LogicalDevice, DescriptorSetLayouts[0],
                                 PALLOCATOR);
    vkDestroyDescriptorSetLayout(LogicalDevice, DescriptorSetLayouts[1],
                                 PALLOCATOR);
    vkDestroyDescriptorSetLayout(LogicalDevice, DescriptorSetLayouts[2],
                                 PALLOCATOR);
    vkDestroyDescriptorSetLayout(LogicalDevice, DescriptorSetLayouts[3],
                                 PALLOCATOR);
  ...
  int FindMemoryThatIsDeviceLocal(uint32_t memoryTypeBits) {
    return FindMemoryByFlagAndType(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                   memoryTypeBits);
  }

  int FindMemoryThatIsHostVisible(uint32_t memoryTypeBits) {
    return FindMemoryByFlagAndType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                                   memoryTypeBits);
  }

  int FindMemoryByFlagAndType(VkMemoryPropertyFlagBits memoryFlagBits,
    ...
    fprintf(FpDebug,
            "\n***** Could not find given memory flag (0x%08x) and type "
            "(0x%08x) *****\n",
  ...
  int FindQueueFamilyThatDoesGraphics() {
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(
        IN PhysicalDevice, &count, OUT(VkQueueFamilyProperties *) nullptr);
    VkQueueFamilyProperties *vqfp = new VkQueueFamilyProperties[count];
    vkGetPhysicalDeviceQueueFamilyProperties(IN PhysicalDevice, &count,
                                             OUT vqfp);
    for (unsigned int i = 0; i < count; i++) {
      if ((vqfp[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) != 0) {
        delete[] vqfp;
  ...
  VkResult RenderScene() {
    ...
    vcbbi.pNext = nullptr;
    vcbbi.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    // vcbbi.flags = VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT;   <----- or
    // could use this one??
    vcbbi.pInheritanceInfo = (VkCommandBufferInheritanceInfo *)nullptr;
    ...
    vkCmdSetViewport(CommandBuffers[nextImageIndex], 0, 1,
                     IN & viewport); // 0=firstViewport, 1=viewportCount
    ...
    vkCmdBindDescriptorSets(
        CommandBuffers[nextImageIndex], VK_PIPELINE_BIND_POINT_GRAPHICS,
        GraphicsPipelineLayout, 0, 4, DescriptorSets, 0, (uint32_t *)nullptr);
    // dynamic offset count, dynamic offsets
    // vkCmdBindPushConstants( CommandBuffers[nextImageIndex], PipelineLayout,
    // VK_SHADER_STAGE_ALL, offset, size, void *values );
    ...
    vkCmdBindVertexBuffers(CommandBuffers[nextImageIndex], 0, 1, buffers,
                           offsets);

    const uint32_t vertexCount = sizeof(VertexData) / sizeof(VertexData[0]);
    // const uint32_t indexCount = sizeof(JustIndexData) /
    // sizeof(JustIndexData[0]);
    ...
    vkCmdDraw(CommandBuffers[nextImageIndex], vertexCount, instanceCount,
    ...
    result = vkQueueSubmit(presentQueue, 1, IN & vsi,
                           IN renderFence); // 1 = submitCount
    if (Verbose && NumRenders <= 2)
      REPORT("vkQueueSubmit");

    result = vkWaitForFences(LogicalDevice, 1, IN & renderFence, VK_TRUE,
                             UINT64_MAX); // waitAll, timeout
    if (Verbose && NumRenders <= 2)
      REPORT("vkWaitForFences");
  ...
  void Reset() {
    ...
    Scene.uProjection =
        glm::perspective(FOV, (double)Width / (double)Height, 0.1, 1000.);
  ...
  void UpdateScene() {
    ...
    if (UseRotate) {
      if (!Paused) {
        ...
        Scene.uSceneOrient = glm::rotate(
            glm::mat4(), (float)glm::radians(360.f * Time / SECONDS_PER_CYCLE),
            axis);
        Scene.uSceneOrient =
            glm::scale(Scene.uSceneOrient, glm::vec3(Scale, Scale, Scale));
      }
      ...
      Scene.uSceneOrient =
          glm::rotate(Scene.uSceneOrient, Yrot, glm::vec3(0., 1., 0.));
      Scene.uSceneOrient =
          glm::rotate(Scene.uSceneOrient, Xrot, glm::vec3(1., 0., 0.));
    ...
    Object.uNormal = glm::mat4(
        glm::inverseTranspose(glm::mat3(Scene.uSceneOrient * Object.uModel)));
  ...
  void InitGLFWSurface() {
    VkResult result =
        glfwCreateWindowSurface(Instance, MainWindow, NULL, &Surface);
  ...
  void GLFWKeyboard(GLFWwindow * window, int key, int scancode, int action,
                    int mods) {
