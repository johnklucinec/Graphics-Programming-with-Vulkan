...
main()
{
    ...
    if (Sporadic.uUseLighting != 0)
    {
        ...
        vec3 normalTexSample = texture(uNormalTex, vTexCoord).rgb;
        vec3 tangentSpaceNormal = 2.0 * normalTexSample - 1.0;
        tangentSpaceNormal.x= -tangentSpaceNormal.x;		// Flip the X so shadows go down and not up
        // tangentSpaceNormal.y = -tangentSpaceNormal.y;
        tangentSpaceNormal.z = tangentSpaceNormal.z * 0.9;	// 0.9 is the "depth"

        // vec3 normal = normalize(vN);,
        vec3 normal = normalize(vN + tangentSpaceNormal);
